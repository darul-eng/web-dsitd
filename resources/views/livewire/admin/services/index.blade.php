<div class="space-y-4 font-inter">
    ...
    <div class="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex flex-col lg:flex-row items-center gap-4">
        <div class="px-2 shrink-0 border-r border-slate-100 pr-4 hidden xl:block">
            <h1 class="text-lg font-extrabold text-slate-900 tracking-tight whitespace-nowrap">Direktori Layanan</h1>
        </div>
        <div class="xl:hidden shrink-0">
            <h1 class="text-lg font-extrabold text-slate-900 tracking-tight whitespace-nowrap pr-2">Layanan</h1>
        </div>
        ...
        <div class="relative flex-grow w-full group">
            ...
            <input wire:model.live="search" type="text" placeholder="Cari layanan..." 
            ...
            <table class="w-full text-left border-collapse">
                <thead class="bg-slate-50/50">
                    <tr class="border-b border-slate-200">
                        <th class="px-5 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Detail Layanan</th>
                        <th class="px-5 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center">Status</th>
                        <th class="px-5 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center">Urutan</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-all group">
                        <td class="px-5 py-2.5">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-lg bg-slate-100 flex-shrink-0 overflow-hidden border border-slate-200 flex items-center justify-center">
                                    @if($item->icon)
                                    <img src="{{ asset('storage/' . $item->icon) }}" class="w-full h-full object-cover">
                                    @else
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="text-slate-300"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"></path><polyline points="3.27 6.96 12 12.01 20.73 6.96"></polyline><line x1="12" y1="22.08" x2="12" y2="12"></line></svg>
                                    @endif
                                </div>
                                <div class="min-w-0">
                        ...
                        <td class="px-5 py-2.5 text-center">
                            <button wire:click="toggleStatus('{{ $item->uuid }}')" class="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold border transition-colors {{ $item->is_active ? 'bg-green-50 text-green-700 border-green-100 hover:bg-green-100' : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100' }}">
                                {{ $item->is_active ? 'Aktif' : 'Non-aktif' }}
                            </button>
                        </td>
                        <td class="px-5 py-2.5 text-center">
                            <span class="text-xs font-extrabold text-slate-700">#{{ $item->order }}</span>
                        </td>
                        <td class="px-5 py-2.5">
                            <div class="flex items-center justify-end gap-1">
                                <a href="{{ route('admin.services.edit', $item->uuid) }}" 
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-center">
                            <p class="text-sm font-bold text-slate-400 tracking-tight italic">Belum ada layanan...</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
